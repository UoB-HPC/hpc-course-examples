...
int main(int argc, char* argv[])
{
  ...
  MPI_File_open(MPI_COMM_WORLD, "iwrite.txt",
  ...
  MPI_File_iwrite(file_handle, buf, BUFSIZE, MPI_CHAR, &request);

  /* >>> we could get on with something else here <<< */

  /* we must wait for the async request to be completed */
